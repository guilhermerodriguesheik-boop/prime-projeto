...
const AdminFixedExpenses: React.FC<AdminFixedExpensesProps> = ({ fixedExpenses, onUpdateExpenses, onBack }) => {
  ...
  // CORREÇÃO: Garante soma numérica real
          ...
          <Card className="border-indigo-900/30 sticky top-24">
            ...
            <form onSubmit={handleAddExpense} className="space-y-4">
              <Select 
                ...
                required
              />
              <Input label="Mês de Competência" type="month" value={mes} onChange={setMes} required />
              <Input label="Descrição" value={descricao} onChange={setDescricao} required placeholder="Ex: Salário Administração" />
              <Input label="Valor (R$)" type="number" value={valor} onChange={setValor} required placeholder="0.00" />
              <div className="pt-2">
                <BigButton onClick={() => {}} variant="primary" disabled={!descricao || !valor}>LANÇAR DESPESA</BigButton>
              </div>
            </form>
          </Card>
        </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="space-y-3">
            {filteredExpenses.length === 0 ? (
              <div className="py-20 text-center text-slate-600 border border-dashed border-slate-800 rounded-2xl italic">Nenhuma despesa fixa lançada para este mês.</div>
            ) : (
                ...
                <Card key={expense.id} className="flex items-center justify-between gap-4 hover:border-slate-700 transition-all py-4">
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 bg-slate-950 rounded-xl flex items-center justify-center text-xl shadow-inner border border-slate-800">
                      {expense.categoria === 'funcionario' ? '👥' : expense.categoria === 'contador' ? '📑' : expense.categoria === 'manobra' ? '🚜' : expense.categoria === 'sistema' ? '💻' : expense.categoria === 'imposto' ? '🏛️' : '💰'}
                    </div>
                    <div>
