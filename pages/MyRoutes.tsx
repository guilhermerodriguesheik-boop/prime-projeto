      ...
      <div className="grid grid-cols-1 gap-4">
        {routes.length === 0 && (
          <div className="text-center py-20 bg-slate-900/30 border border-dashed border-slate-800 rounded-2xl">
            ...
            <p className="text-slate-600 font-bold uppercase text-xs tracking-widest">Nenhuma rota registrada para você</p>
          </div>
        )}
        
        {routes.sort((a,b) => b.createdAt.localeCompare(a.createdAt)).map(r => (
          <Card key={r.id} className="border-l-4 border-l-blue-600">
            <div className="flex justify-between items-start mb-4">
              <div className="flex items-center gap-2">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-xs font-bold text-white">{r.oc}</span>
                </div>
              </div>
            ...
            <div className="bg-slate-950/50 p-4 rounded-xl border border-slate-800 mb-4">
               <div className="flex items-center gap-4 text-sm">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-slate-200 font-bold">{r.destino}</div>
                </div>
              </div>
            </div>
            ...
            <div className="flex justify-between items-center text-[10px] text-slate-500 font-bold uppercase tracking-widest">
              ...
              {r.valorAjudante ? (
                <span className="text-emerald-500">Saldo: R$ {r.valorAjudante.toLocaleString()}</span>
              ) : (
                <span className="italic">Aguardando Financeiro</span>
              )}
            </div>
          </Card>
        ))}
